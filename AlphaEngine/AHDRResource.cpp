...
std::shared_ptr<AResource> DXHDRResource::GetRTVResource(int index)
{
	mResource->Resource = mHDRs[index].Get();
	return mResource;
}

std::shared_ptr<AResource> DXHDRResource::GetDSVResource(int index)
{
...
void DXHDRResource::BuildDescriptors(CD3DX12_CPU_DESCRIPTOR_HANDLE hCpuSrv, CD3DX12_CPU_DESCRIPTOR_HANDLE hCpuDsv, CD3DX12_CPU_DESCRIPTOR_HANDLE hCpuRtv, int index, int width, int height)
{
...
void DXHDRResource::BuildDescriptors(int index)
{
	...
	ARHIDX12::GetSingleton().GetDevice()->CreateShaderResourceView(mHDRs[index].Get(), &srvDesc, mhCpuSrv[index]);
	D3D12_RENDER_TARGET_VIEW_DESC rtvDesc = {};
	rtvDesc.Format = DXGI_FORMAT_R16G16B16A16_FLOAT;
	rtvDesc.ViewDimension = D3D12_RTV_DIMENSION_TEXTURE2D;
	rtvDesc.Texture2D.MipSlice = 0;
	rtvDesc.Texture2D.PlaneSlice = 0;
	ARHIDX12::GetSingleton().GetDevice()->CreateRenderTargetView(mHDRs[index].Get(), &rtvDesc, mhCpuRtv[index]);
	...
	ARHIDX12::GetSingleton().GetDevice()->CreateDepthStencilView(mShadows[index].Get(), &dsvDesc, mhCpuDsv[index]);
}
